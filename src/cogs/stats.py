...
from common.plotting import create_plot, create_scores_plot, create_tiers_plot
...
class Stats(commands.Cog):
    ...
    async def scores(
        ...
        embed.add_field(
            name="Average Score", value=f"{avg_score:.1f}", inline=True
        )
        embed.add_field(name="Top Score", value=f"{top_score}", inline=True)
        embed.add_field(name="​", value="​", inline=True)

        embed.add_field(
            name="Median Score", value=f"{median_score:g}", inline=True
        )
        embed.add_field(
            name="Standard Deviation", value=f"{std_dev:.1f}", inline=True
        )
        embed.add_field(name="​", value="​", inline=True)
